...
class FCN_32s(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        fully_conv = list(classifier.children())
...
fcn = FCN_32s(num_classes=21)
fcn.load_state_dict(torch.load('fcn_32s_1000ep_2nd.pth'))
fcn.cuda()
...
for epoch in range(1000):  # loop over the dataset multiple times
    ...
    if current_validation_score > best_validation_score:
        ...
        torch.save(fcn.state_dict(), 'fcn_32s_1000ep_3nd.pth')
        best_validation_score = current_validation_score
            
    print('epoch %i; Dev acc %f; Best dev acc %f;' % (epoch, current_validation_score, best_validation_score))
